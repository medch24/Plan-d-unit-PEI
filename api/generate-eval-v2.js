...
function createEvaluationDocument(data) {
    ...
    exercices.forEach((ex, idx) => {
        ...
        sections.push(
            new Paragraph({
                children: [new TextRun({ text: ex.description, size: 22 })],
                spacing: { after: 200 }
            })
        );
        ...
        sections.push(
            new Paragraph({
                children: [new TextRun({ text: "Réponse :", size: 22 })],
                spacing: { after: 100 }
            })
        );
...
export default async function handler(req, res) {
    ...
    try {
        ...
        console.log('[INFO] Generate Eval V2 Request received');
        ...
        if (!criteres || criteres.length === 0) {
            ...
            console.log('[INFO] Critères auto-sélectionnés:', criteres);
        }
        
        // Utiliser le premier critère
